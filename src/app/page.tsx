...
export default function Home() {
  ...
  // Section background — semi-transparent so video peeks through
  const sectionBg = (opacity: number) => `rgba(7,7,10,${opacity})`;
  ...
  return (
    <>
      ...
      {/* ═══ FIXED VIDEO BACKGROUND — Drives through the ENTIRE page ═══ */}
      <video
        ref={videoRef}
        muted
        playsInline
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 1, background: "radial-gradient(ellipse at center, transparent 30%, rgba(7,7,10,0.5) 100%)", pointerEvents: "none" }} />
      ...
      {/* ═══ ALL CONTENT — sits on top of the fixed video ═══ */}
      <div style={{ position: "relative", zIndex: 2 }}>

        {/* ═══ SCENE 1: HERO — Fully transparent, video shows through ═══ */}
        <section className="hero-section" style={{ position: "relative", height: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {/* Light gradient just for text readability */}
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, rgba(7,7,10,0.35) 0%, rgba(7,7,10,0.05) 40%, rgba(7,7,10,0.05) 60%, rgba(7,7,10,0.45) 100%)", pointerEvents: "none" }} />
          ...
          <div className="hero-text" style={{ position: "relative", zIndex: 10, textAlign: "center", padding: "0 24px" }}>
            ...
            <p style={{ fontSize: "clamp(0.9rem, 1.3vw, 1.08rem)", color: "rgba(240,236,228,0.8)", lineHeight: 1.7, maxWidth: 540, margin: "0 auto 44px", textShadow: "0 2px 20px rgba(0,0,0,0.7)" }}>
          ...
          <div style={{ position: "absolute", bottom: 32, left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, opacity: 0.4, animation: "bob 2.5s ease-in-out infinite" }}>
            <span style={{ fontSize: "0.5rem", letterSpacing: "0.2em", color: "var(--text3)", textTransform: "uppercase" }}>Keep scrolling — the drive never stops</span>
            ...
            <div className="reveal-stagger" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: 20 }}>
              {CASES.map((c, i) => <div key={i} style={{ background: "rgba(20,20,28,0.9)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden", transition: "all 0.5s", backdropFilter: "blur(8px)" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-8px)"; e.currentTarget.style.boxShadow = "0 20px 60px rgba(0,0,0,0.5)"; }} onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = ""; }}>
                <div style={{ height: 180, overflow: "hidden", position: "relative" }}>
                  ...
                  <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(20,20,28,0.95), transparent 60%)" }} />
        ...
        <section id="contact" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "clamp(80px, 14vh, 160px) clamp(20px, 6vw, 80px)", background: sectionBg(0.3) }}>
          ...
          {!formSubmitted ? <>
          ...
          </> : <div style={{ textAlign: "center", animation: "fadeInUp 0.8s ease-out", position: "relative", zIndex: 2 }}>
            ...
            <h2 style={{ fontFamily: "var(--serif)", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 300, marginBottom: 14, textShadow: "0 2px 20px rgba(0,0,0,0.5)" }}>Welcome.</h2>

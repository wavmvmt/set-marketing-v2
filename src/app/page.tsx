...
export default function Home() {
  const [splashPhase, setSplashPhase] = useState(0); // 0=logo, 1=title, 2=loading, 3=enter, 4=fadeout, 5=done
  ...
  const enterSite = () => {
    setSplashPhase(4);
    setTimeout(() => setSplashPhase(5), 1000);
  };

  // ── GSAP: Scroll-driven hero video + Lenis + Method gates ──
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // ═══ HERO DRIFT VIDEO — Scroll-driven, pinned ═══
      ...
      if (heroVideo && heroSection) {
        ...
        const waitForVideo = () =>
          new Promise<void>((resolve) => {
            if (heroVideo.readyState >= 1) return resolve();
        ...
        // GSAP ScrollTrigger — pin the section, scrub the video
        ScrollTrigger.create({
          trigger: heroSection,
          start: "top top",
          end: "bottom bottom",
          scrub: 0, // direct link, we handle smoothing ourselves
          pin: ".hero-video-sticky",
          anticipatePin: 1,
          onUpdate: (self) => {
        ...
        // Smooth lerp loop for video seeking
        let renderTime = 0;
        const tick = () => {
          renderTime += (currentTime - renderTime) * 0.12;
          if (Math.abs(renderTime - heroVideo.currentTime) > 0.015) {
            heroVideo.currentTime = renderTime;
          }
    ...
    <>
        ...
        >
          ...
          <div style={{ position: "relative", zIndex: 10, textAlign: "center", padding: "0 24px", maxWidth: 800 }}>
            ...
            <div style={{ opacity: splashPhase >= 1 ? 1 : 0, transform: splashPhase >= 1 ? "translateY(0)" : "translateY(20px)", transition: "all 1s ease" }}>
              <div style={{ fontFamily: "var(--sans)", fontSize: "0.65rem", letterSpacing: "0.35em", color: "var(--gold)", textTransform: "uppercase", marginBottom: 20, fontWeight: 500, textShadow: "0 0 20px rgba(200,160,80,0.4)" }}>
            ...
            {/* Loading */}
            <div style={{ marginTop: 48, opacity: splashPhase === 2 ? 1 : 0, display: splashPhase === 2 ? "block" : "none", transition: "opacity 0.6s" }}>
              <span style={{ fontFamily: "var(--sans)", fontSize: "0.75rem", letterSpacing: "0.2em", color: "var(--text3)", textTransform: "uppercase" }}>
                Loading<span style={{ display: "inline-block" }}><span style={{ animation: "dotPulse 1.4s infinite" }}>.</span><span style={{ animation: "dotPulse 1.4s infinite 0.2s" }}>.</span><span style={{ animation: "dotPulse 1.4s infinite 0.4s" }}>.</span></span>
              </span>
            </div>

            {/* Enter */}
            <div style={{ marginTop: 48, opacity: splashPhase >= 3 && splashPhase < 4 ? 1 : 0, display: splashPhase >= 3 && splashPhase < 5 ? "block" : "none" }}>
              <div onClick={enterSite} style={{ fontFamily: "var(--sans)", fontSize: "0.85rem", letterSpacing: "0.25em", color: "var(--gold)", textTransform: "uppercase", cursor: "pointer", animation: "flashPulse 1.5s ease-in-out infinite", textShadow: "0 0 20px rgba(200,160,80,0.4)", padding: "16px 32px", border: "1px solid rgba(200,160,80,0.25)", borderRadius: 6, display: "inline-block" }}
      ...
      {splashPhase >= 5 && (
        <>
          ...
          {/* ═══ HERO: SCROLL-DRIVEN DRIFT VIDEO ═══
              The video already contains the title text + car smashing it.
              We just present it fullscreen, pinned, scroll-scrubbed.
              Section is 500vh tall — scroll distance controls video playback.
              The sticky container holds the video in viewport while you scroll through. */}
          <section ref={heroSectionRef} style={{ position: "relative", height: "500vh" }}>
            <div className="hero-video-sticky" style={{ position: "sticky", top: 0, width: "100%", height: "100vh", overflow: "hidden", background: "#000" }}>
              <video
                ref={heroVideoRef}
                muted
                playsInline
              ...
              <div style={{ position: "absolute", bottom: 32, left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, opacity: 0.4, animation: "bob 2.5s ease-in-out infinite", zIndex: 10 }}>
                <span style={{ fontSize: "0.5rem", letterSpacing: "0.2em", color: "#fff", textTransform: "uppercase", textShadow: "0 1px 8px rgba(0,0,0,0.8)" }}>Scroll</span>
                <div style={{ width: 1, height: 24, background: "linear-gradient(to bottom, rgba(255,255,255,0.5), transparent)" }} />
              </div>
            </div>
          </section>

          {/* ═══ AFTER THE DRIFT: Rest of the site flows naturally ═══ */}
